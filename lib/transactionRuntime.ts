...
async function sendWhatsApp(db:any,item:any,template:any){
 ...
 let payload:any
 if(templateName){payload={messaging_product:'whatsapp',to,type:'template',template:{name:templateName,language:{code:template.provider_template_language||'en'}}}}
 else if(cfg.public_config?.allow_session_text===true){payload={messaging_product:'whatsapp',to,type:'text',text:{body:bodyText}}}
...
export async function processCommunicationQueue(limit=20){
 ...
 const {data:rows,error}=await db.rpc('claim_communication_outbox',{p_limit:Math.max(1,Math.min(limit,50))})
 if(error)throw error
 const results:any[]=[]
 for(const item of rows||[]){
  const {data:template,error:templateError}=await db.from('communication_templates').select('*').eq('template_key',item.template_key).maybeSingle()
  try{
   if(templateError)throw templateError
   if(!template||!template.is_active)throw new Error(`Communication template ${item.template_key} is unavailable or inactive.`)
   let external=''
   ...
   await db.from('communication_outbox').update({status:'sent',provider_message_id:external||null,sent_at:now,last_error:null,locked_at:null,lock_token:null,updated_at:now}).eq('id',item.id).eq('lock_token',item.lock_token)
   await log(db,{integration_key:item.channel==='email'?'email':item.channel==='whatsapp'?'whatsapp':'sms',direction:'outbound',event_type:item.event_key,status:'sent',external_id:external||null,reference_type:item.reference_type,reference_id:item.reference_id,response_metadata:{template_key:item.template_key},created_at:now})
   results.push({id:item.id,status:'sent'})
  }catch(error:any){
   const attempts=Number(item.attempts||1),final=attempts>=Number(item.max_attempts||3),now=new Date().toISOString()
   await db.from('communication_outbox').update({status:final?'failed':'queued',available_at:new Date(Date.now()+Math.min(attempts*5,30)*60000).toISOString(),last_error:String(error?.message||error),locked_at:null,lock_token:null,updated_at:now}).eq('id',item.id).eq('lock_token',item.lock_token)
   await log(db,{integration_key:item.channel==='email'?'email':item.channel==='whatsapp'?'whatsapp':'sms',direction:'outbound',event_type:item.event_key,status:'failed',reference_type:item.reference_type,reference_id:item.reference_id,error_message:String(error?.message||error),created_at:now})
