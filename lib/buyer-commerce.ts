...
export type BuyerCartItem={id:string;kind:'standard'|'custom';name:string;variant:string;price:number;qty:number;gstRate?:number|null;minQty?:number;quantityStep?:number;productVariantId?:string;productSlug?:string;imageUrl?:string;imageAlt?:string;customType?:'acdb'|'dcdb';templateId?:string;selectedValueIds?:string[];selectedOptions?:Record<string,any>;visualSelections?:{value_id:string;quantity:number;option_key?:string}[];visualPreview?:Record<string,any>}
// A hard ceiling on distinct lines, independent of the 500-line hydration cap below,
// so repeated additions of new SKUs cannot grow an already-saved cart without bound.
...
export function mergeCart(items:BuyerCartItem[],item:BuyerCartItem,maxQty?:number):{ok:true;items:BuyerCartItem[]}|{ok:false;error:string} {
  ...
  if(!existing&&items.length>=MAX_CART_LINES)return{ok:false,error:'Your cart has reached its maximum number of items. Remove an item before adding another.'}
  const qty=rounded((existing?.qty||0)+item.qty,6)
...
// Applies several additions as one all-or-nothing batch (e.g. an ACDB+DCDB combo),
// so a mid-batch failure cannot leave only some of a related group in the cart.
export function mergeCartMany(items:BuyerCartItem[],additions:{item:BuyerCartItem;maxQty?:number}[]):{ok:true;items:BuyerCartItem[]}|{ok:false;error:string} {
  let next=items
  for(const {item,maxQty} of additions){
    const result=mergeCart(next,item,maxQty)
...
function optionalString(v:unknown):boolean {return v===undefined||typeof v==='string'}
function optionalPositive(v:unknown):boolean {return v===undefined||(typeof v==='number'&&Number.isFinite(v)&&v>0&&v<=MAX_BUYER_QUANTITY)}
// A saved line is dropped outright (never silently coerced) when any present field is
// the wrong shape: a corrupt optional field cannot be trusted to compute the right price.
function validLine(i:any):i is BuyerCartItem {
  if(!i||typeof i!=='object')return false
  if(!nonEmptyString(i.id)||(i.kind!=='standard'&&i.kind!=='custom'))return false
  if(!nonEmptyString(i.name)||!nonEmptyString(i.variant))return false
  if(!Number.isFinite(i.price)||i.price<=0||i.price>1e9)return false
  if(!Number.isFinite(i.qty)||i.qty<=0||i.qty>MAX_BUYER_QUANTITY)return false
  if(!(i.gstRate===undefined||i.gstRate===null||(typeof i.gstRate==='number'&&Number.isFinite(i.gstRate)&&i.gstRate>=0&&i.gstRate<=100)))return false
  if(!optionalPositive(i.minQty)||!optionalPositive(i.quantityStep))return false
  for(const key of ['productVariantId','productSlug','imageUrl','imageAlt','templateId'])if(!optionalString(i[key]))return false
  if(!(i.customType===undefined||i.customType==='acdb'||i.customType==='dcdb'))return false
  if(!(i.selectedValueIds===undefined||(Array.isArray(i.selectedValueIds)&&i.selectedValueIds.every(nonEmptyString))))return false
  if(!(i.visualSelections===undefined||(Array.isArray(i.visualSelections)&&i.visualSelections.every((x:any)=>x&&typeof x==='object'&&nonEmptyString(x.value_id)&&typeof x.quantity==='number'&&Number.isFinite(x.quantity)&&x.quantity>0&&optionalString(x.option_key)))))return false
  if(!(i.selectedOptions===undefined||(typeof i.selectedOptions==='object'&&i.selectedOptions!==null&&!Array.isArray(i.selectedOptions))))return false
  if(!(i.visualPreview===undefined||(typeof i.visualPreview==='object'&&i.visualPreview!==null&&!Array.isArray(i.visualPreview))))return false
  // A custom build needs enough of its own configuration to be re-priced later; without it the
  // line would only fail confusingly at checkout, so it is dropped here instead.
  if(i.kind==='custom'&&!(nonEmptyString(i.templateId)&&((Array.isArray(i.selectedValueIds)&&i.selectedValueIds.length>0)||(Array.isArray(i.visualSelections)&&i.visualSelections.length>0))))return false
  return true
}
export function readCart(raw:string|null):BuyerCartItem[] {
  try{const data=JSON.parse(raw||'[]');if(!Array.isArray(data))return[]
    return data.filter(validLine).slice(0,MAX_CART_LINES)
  }catch{return[]}
}
