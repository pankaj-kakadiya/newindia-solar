...
export function spriteFor(settings?:Record<string,any>):Sprite|undefined {
  return builderAssets.find(a=>a.id===settings?.asset_id)
}
export function imageSource(item:any):string {
  return spriteFor(item?.visual_settings)?.src || item?.inside_image_url || item?.image_url || ''
}
export function safeImageSource(src:string):boolean {
  return /^\/(?!\/)/.test(src)||/^https:\/\//.test(src)
}
...
// Reserve slots across all selected parts, so multi-select accessories never stack in slot 1.
...
export function requiresQuote(template:any,items:any[]):boolean{
  return template?.preview_settings?.commerce_mode==='quote'||items.some(item=>!item||item.visual_settings?.pricing_status==='quote_required'||!(Number(item.selling_price)>0))
}
export function quoteDraft(code:string,summary:string,selections:unknown){
  return {version:1,product:code,summary:summary.slice(0,12000),selections,createdAt:Date.now()}
}
