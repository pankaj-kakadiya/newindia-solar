...
export default async function CmsPage({params}:{params:Promise<{slug:string}>}){const {slug}=await params;const p=await getPage(slug);if(!p)notFound();return <><StoreHeader/><main className="cmsPublicPage"><div className="container"><div className="cmsPublicHead"><span>NEW INDIA SOLAR</span><h1>{p.title}</h1>{p.excerpt&&<p>{p.excerpt}</p>}</div><article className="cmsPublicBody" dangerouslySetInnerHTML={{__html:p.body_html||''}}/></div></main><StoreFooter/></>}
