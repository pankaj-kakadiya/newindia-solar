...
export async function GET(request:NextRequest){try{
 const {user,client}=await buyerContext(request)
 ...
 const [profile,addresses,orders]=await Promise.all([
  client.from('profiles').select(PROFILE_FIELDS).eq('id',user.id).maybeSingle().throwOnError(),
  client.from('addresses').select(ADDRESS_FIELDS).eq('user_id',user.id).order('is_default_shipping',{ascending:false}).order('created_at',{ascending:false}).limit(100).throwOnError(),
  client.from('orders').select(ORDER_FIELDS).eq('user_id',user.id).order('created_at',{ascending:false}).range(offset,offset+19).throwOnError()
 ])
