...
import {staffRoles,validateManualTeamUser,validateTemporaryPassword} from '../../../../lib/teamUser'
...
export async function GET(request:NextRequest){
 ...
 const profiles:any[]=[]
 for(let i=0;i<ids.length;i+=200){const part=ids.slice(i,i+200);const {data:rows,error:profileError}=await admin.from('profiles').select('id,full_name,email,phone,role,admin_role,staff_status,job_title,account_status,created_at,updated_at').in('id',part);if(profileError)return response({error:profileError.message},503);profiles.push(...(rows||[]))}
 const profileById=new Map(profiles.map(profile=>[profile.id,profile]))
 const users=authUsers.map(user=>{const profile=profileById.get(user.id)||{};return {id:user.id,email:user.email||profile.email||null,phone:user.phone||profile.phone||null,full_name:profile.full_name||null,role:profile.role||'customer',admin_role:profile.admin_role||null,staff_status:profile.staff_status||'active',job_title:profile.job_title||null,account_status:profile.account_status||'active',email_confirmed:Boolean(user.email_confirmed_at),last_sign_in_at:user.last_sign_in_at||null,created_at:user.created_at,updated_at:user.updated_at,providers:Array.isArray(user.app_metadata?.providers)?user.app_metadata.providers:[],banned:Boolean(user.banned_until&&new Date(user.banned_until).getTime()>Date.now())}})
 return response({users,total:users.length})
}
...
export async function POST(request:NextRequest){
 ...
 const {data:profile,error:profileError}=await admin.from('profiles').upsert({id:created.user.id,full_name:input.full_name,email:input.email,phone:input.phone,role:'staff',admin_role:input.admin_role,staff_status:'active',job_title:input.job_title,updated_at:new Date().toISOString()},{onConflict:'id'}).select('id').single()
...
export async function PATCH(request:NextRequest){
 ...
 if(body.action==='reset_password'){
  ...
  return error?response({error:error.message},422):response({message:'Temporary password updated. Existing passwords can never be viewed.'})
 }
 ...
 if(role==='staff'&&!staffRoles.includes(adminRole as any))return response({error:'Invalid department role.'},400)
