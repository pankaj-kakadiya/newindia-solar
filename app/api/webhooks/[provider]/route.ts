...
export async function POST(request:NextRequest,{params}:{params:Promise<{provider:string}>}){
 ...
 let json:any={};try{json=JSON.parse(body)}catch{return NextResponse.json({error:'Invalid JSON.'},{status:400})}
 ...
 let {data:webhook,error:webhookError}=await db.from('integration_webhook_events').insert({integration_key:cfg.key,provider_event_id:externalId||null,dedupe_key:dedupeKey,event_type:eventType,signature_valid:valid,processing_status:valid?'received':'failed',payload_hash:hash,safe_headers:safeHeaders,error_message:valid?null:'Invalid webhook signature'}).select('id').single()
 if(webhookError?.code==='23505'){
  const {data:prior,error:priorError}=await db.from('integration_webhook_events').select('id,processing_status,payload_hash,signature_valid').eq('integration_key',cfg.key).eq('dedupe_key',dedupeKey).single()
  if(priorError||!prior)return NextResponse.json({error:'Could not read webhook receipt.'},{status:503})
  if(prior.signature_valid&&prior.payload_hash!==hash)return NextResponse.json({error:'Webhook delivery payload mismatch.'},{status:409})
  if(prior.signature_valid&&prior.processing_status==='processed')return NextResponse.json({received:true,duplicate:true},{headers:{'Cache-Control':'no-store'}})
  // Retry received/failed events, including receipts left by interrupted requests.
  // Repeated delivery uses absolute values; paid payment rows cannot become pending.
  webhook={id:prior.id};webhookError=null
  const {error:retryError}=await db.from('integration_webhook_events').update({signature_valid:true,payload_hash:hash,error_message:null,processing_status:'received'}).eq('id',prior.id)
  if(retryError)return NextResponse.json({error:'Could not retry webhook receipt.'},{status:503})
 }
 ...
 try{
  if(provider==='razorpay'){
   ...
   if(providerOrderId&&providerPaymentId&&['payment.captured','payment.authorized'].includes(eventType)){
    const {data:p,error:paymentError}=await db.from('payments').select('id,order_id,amount').eq('provider','razorpay').eq('provider_order_id',providerOrderId).maybeSingle()
    if(paymentError||!p)throw new Error('Payment record is not available yet.')
    if(p){
     const expectedPaise=paymentPaise(p.amount)
     if(!matchesPayment(entity,providerOrderId,providerPaymentId,expectedPaise)||entity.status!==(eventType==='payment.captured'?'captured':'authorized'))throw new Error('Razorpay webhook amount does not match the payment record.')
     const paidStatus=eventType==='payment.captured'?'paid':'pending',now=new Date().toISOString()
     await db.from('payments').update({provider_payment_id:providerPaymentId,status:paidStatus,received_at:eventType==='payment.captured'?now:null,payment_method:'online',reference_no:providerPaymentId,raw_response:{provider_order_id:providerOrderId,provider_payment_id:providerPaymentId,event:eventType,provider_status:entity?.status||null,amount_verified:true},updated_at:now}).eq('id',p.id).in('status',['pending','failed',...(paidStatus==='paid'?['paid']:[])]).throwOnError()
     if(eventType==='payment.captured'){
      const {data:o}=await db.from('orders').select('grand_total').eq('id',p.order_id).single().throwOnError(),{data:rows}=await db.from('payments').select('amount').eq('order_id',p.order_id).eq('status','paid').throwOnError()
      ...
      await db.from('orders').update({payment_status:status,updated_at:now}).eq('id',p.order_id).throwOnError()
      await db.from('invoices').update({amount_paid:paid,balance_due:Math.max(0,orderTotal-paid),updated_at:now}).eq('order_id',p.order_id).neq('status','void').throwOnError()
     }
    }
   }
  }
  if(provider==='whatsapp'){
   ...
   if(messageId&&status){
    ...
    await db.from('communication_outbox').update({status:next,delivered_at:next==='delivered'?new Date().toISOString():null,last_error:status==='failed'?String(statusRow?.errors?.[0]?.title||'WhatsApp delivery failed'):null,updated_at:new Date().toISOString()}).eq('provider_message_id',messageId).throwOnError()
   }
  }
  if(webhook?.id)await db.from('integration_webhook_events').update({processing_status:'processed',error_message:null,processed_at:new Date().toISOString()}).eq('id',webhook.id).throwOnError()
  ...
  return NextResponse.json({received:true,processed:false},{status:503,headers:{'Cache-Control':'no-store'}})
 }
}
