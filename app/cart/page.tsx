...
export default function Cart(){
  ...
  useEffect(()=>{const ids=[...new Set(items.filter(item=>item.kind==='standard'&&item.productVariantId).map(item=>item.productVariantId as string))];if(!ids.length)return;let alive=true;supabase.from('product_variants').select('id,stock_qty').in('id',ids).then(({data})=>{if(!alive)return;const next:Record<string,number>={};for(const v of (data||[]) as any[])next[v.id]=Number(v.stock_qty||0);setStock(next)});return()=>{alive=false}},[items])
  const gstAmount=gst(total),grand=total+gstAmount
  return <><StoreHeader/><main className="container cartPage"><div className="cartPageHead"><div><span className="eyebrow darkEye">YOUR ORDER</span><h1>Shopping Cart</h1><p>{items.length?`${items.length} line item${items.length>1?'s':''} ready for review with +18% GST.`:'Your cart is currently empty.'}</p></div><Link className="nisTextLink" href="/shop">Continue shopping →</Link></div>{!items.length?<div className="emptyCart premiumEmpty"><ShoppingBag size={38}/><h2>Your cart is empty.</h2><p>Browse solar components or configure your own ACDB/DCDB.</p><div className="btnRow"><Link className="btn btnPrimary" href="/shop">Shop Products</Link><Link className="btn" href="/customize/dcdb">Build DCDB</Link></div></div>:<div className="cartLayout"><section className="cartItemsPanel">{items.map(i=>{const base=i.price*i.qty,tax=gst(base),incl=base+tax;const itemStock=i.productVariantId?stock[i.productVariantId]:undefined;const overStock=itemStock!==undefined&&i.qty>itemStock;return <article className="cartRow" key={i.id}><div className="cartProduct"><CartMedia item={i} media={i.productVariantId?media[i.productVariantId]:undefined}/><div className="cartInfo"><span className="cartKind">{i.kind==='custom'?'Custom configuration':'Product'}</span><b>{i.name}</b><span>{i.variant}</span><small>Base {money(i.price)} + {GST_RATE}% GST per unit</small>{overStock&&<small className="cartStockWarning">Only {itemStock} in stock — reduce quantity to continue.</small>}</div></div><div className="cartCommerce"><div className="cartQty"><span>Quantity</span><QuantityControl value={i.qty} min={i.minQty||1} max={itemStock!==undefined?Math.max(itemStock,i.minQty||1):undefined} label={`Quantity of ${i.name}`} onChange={q=>updateQty(i.id,q)}/></div><strong className="cartLineTotal">{money(incl)}<small>incl. GST</small></strong><button className="cartRemove" onClick={()=>remove(i.id)}><Trash2 size={15}/> Remove</button></div></article>})}</section><aside className="summary"><h2>Order Summary</h2><div><span>Subtotal before GST</span><b>{money(total)}</b></div><div><span>GST @ {GST_RATE}%</span><b>{money(gstAmount)}</b></div><div><span>Shipping</span><span>Calculated at checkout</span></div><div className="grand"><span>Total incl. GST</span><b>{money(grand)}</b></div><p className="summaryNote">Final price and tax are verified against Supabase during secure order creation.</p><Link className="btn btnPrimary fullBtn" href="/checkout">Proceed to Secure Checkout</Link><Link className="bulkLink" href="/bulk-order">Buying for a project? Request bulk price →</Link></aside></div>}</main><StoreFooter/></>
}
