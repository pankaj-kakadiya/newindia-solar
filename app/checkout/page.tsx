...
export default function Checkout(){
  const {items,total,clear}=useCart();const router=useRouter();const [business,setBusiness]=useState(false);const [user,setUser]=useState<any>(null);const [busy,setBusy]=useState(false);const [message,setMessage]=useState('')
  const displayTax=taxOf(total),displayGrand=total+displayTax
  useEffect(()=>{supabase.auth.getUser().then(({data})=>setUser(data.user||null))},[])
  async function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault();if(!user){router.push('/login?next=/checkout');return}if(!items.length){setMessage('Your cart is empty.');return}setBusy(true);setMessage('');const f=new FormData(e.currentTarget)
    ...
    for(const i of items){
      if(i.kind==='standard'){
        ...
        if(i.visualSelections?.length){
          const r=await supabase.rpc('save_visual_configuration',{p_template_id:i.templateId,p_config_name:i.name,p_selections:i.visualSelections.map(s=>({value_id:s.value_id,quantity:s.quantity})),p_preview_snapshot:i.visualPreview||{}});cfgErr=r.error;cfg=r.data
        }else{
          if(!i.selectedValueIds?.length){setBusy(false);setMessage('Custom configuration is incomplete.');return}
          const r=await supabase.rpc('save_custom_configuration',{p_template_id:i.templateId,p_config_name:i.name,p_selected_value_ids:i.selectedValueIds});cfgErr=r.error;cfg=r.data
        }
