...
export default function Checkout(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      if(!order){
        const result=await supabase.rpc('place_order_from_cart',{p_cart_id:cartId,p_customer_snapshot:customer,p_shipping_address:shipping,p_billing_address:shipping,p_business_purchase:business,p_company_name:business?String(f.get('company')||''):null,p_gstin:business?String(f.get('gstin')||''):null,p_po_number:business?String(f.get('po')||''):null,p_payment_method:paymentMethod,p_notes:'Website order'})
        if(result.error)throw new Error(result.error.message)
