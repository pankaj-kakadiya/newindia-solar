...
function builderDraftKey(){const id=new URLSearchParams(window.location.search).get('draft');return id&&/^[a-zA-Z0-9-]{1,80}$/.test(id)?`nis-builder-rfq-${id}`:'nis-builder-rfq'}
export default function Bulk(){
  ...
  const dirty=useRef(false),submitting=useRef(false),fromBuilder=useRef(false)
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search),slug=params.get('product'),draft=params.get('draft')
    const controller=new AbortController();let alive=true
    if(params.get('source')==='builder'&&draft){
      // Hand-off from the ACDB/DCDB visual builder: a component summary, not a priced or approved BOM.
      try{const raw=sessionStorage.getItem(builderDraftKey());const value=raw?JSON.parse(raw):null
        if(value&&value.version===1&&['ACDB','DCDB'].includes(value.product)&&typeof value.summary==='string'&&Number.isFinite(value.createdAt)&&value.createdAt<=Date.now()+60000&&Date.now()-value.createdAt<=86400000){
          setInterest(value.product);setNote(value.summary.slice(0,20000));fromBuilder.current=true;setContext('Your selected components are included below. Add your contact details to request the quotation.')
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    try{const f=new FormData(e.currentTarget),row={name:f.get('name'),company_name:f.get('company'),mobile:f.get('mobile'),gst_number:f.get('gst'),city:f.get('city'),state:f.get('state'),product_interest:interest,quantity:quantity?Number(quantity):null,expected_purchase:f.get('when'),additional_requirement:note}
      ...
      if(fromBuilder.current)try{sessionStorage.removeItem(builderDraftKey())}catch{}
