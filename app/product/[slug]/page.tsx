...
export default function Product(){
  ...
  return <><StoreHeader/><main className="pdPage">
    ...
    <section className="container pdInfo">
      <div className="pdTabs" role="tablist" aria-label="Product information" onKeyDown={event=>{const keys=['overview','specifications','box','downloads'] as const;let index=keys.indexOf(tab);if(event.key==='ArrowRight')index=(index+1)%keys.length;else if(event.key==='ArrowLeft')index=(index+keys.length-1)%keys.length;else if(event.key==='Home')index=0;else if(event.key==='End')index=keys.length-1;else return;event.preventDefault();setTab(keys[index]);event.currentTarget.querySelectorAll<HTMLButtonElement>('[role="tab"]')[index]?.focus()}}><button role="tab" id="pd-tab-overview" aria-controls="pd-panel-overview" aria-selected={tab==='overview'} tabIndex={tab==='overview'?0:-1} className={tab==='overview'?'active':''} onClick={()=>setTab('overview')}><CheckCircle2/>Overview</button><button role="tab" id="pd-tab-specifications" aria-controls="pd-panel-specifications" aria-selected={tab==='specifications'} tabIndex={tab==='specifications'?0:-1} className={tab==='specifications'?'active':''} onClick={()=>setTab('specifications')}><Wrench/>Specifications</button><button role="tab" id="pd-tab-box" aria-controls="pd-panel-box" aria-selected={tab==='box'} tabIndex={tab==='box'?0:-1} className={tab==='box'?'active':''} onClick={()=>setTab('box')}><Box/>In the Box</button><button role="tab" id="pd-tab-downloads" aria-controls="pd-panel-downloads" aria-selected={tab==='downloads'} tabIndex={tab==='downloads'?0:-1} className={tab==='downloads'?'active':''} onClick={()=>setTab('downloads')}><Download/>Downloads</button></div>
      {tab==='overview'&&<div role="tabpanel" id="pd-panel-overview" aria-labelledby="pd-tab-overview" tabIndex={0} className="pdTabContent pdOverview"><div><h2>Product Overview</h2><p>{p.description||p.short_description||'New India Solar component designed for dependable solar installation projects.'}</p></div><div className="pdMiniList"><h3>Applications</h3>{applications.map((x:string)=><span key={x}><CheckCircle2/>{x}</span>)}</div><div className="pdMiniList"><h3>Key Features</h3>{(features.length?features:['Project-ready build','Quality checked','Reliable solar application']).map((x:string)=><span key={x}><CheckCircle2/>{x}</span>)}</div></div>}
      {tab==='specifications'&&<div role="tabpanel" id="pd-panel-specifications" aria-labelledby="pd-tab-specifications" tabIndex={0} className="pdTabContent"><div className="pdSpecGrid">{Object.entries({...specs,...attrs}).length?Object.entries({...specs,...attrs}).map(([k,val]:any)=><div key={k}><span>{k.replaceAll('_',' ')}</span><b>{String(val)}</b></div>):<p>No structured specifications have been added yet.</p>}</div></div>}
      {tab==='box'&&<div role="tabpanel" id="pd-panel-box" aria-labelledby="pd-tab-box" tabIndex={0} className="pdTabContent"><div className="pdBoxList">{p.inclusions?.length?p.inclusions.map((x:string)=><span key={x}><PackageCheck/>{x}</span>):<p>Box contents will be confirmed for this product.</p>}</div></div>}
      {tab==='downloads'&&<div role="tabpanel" id="pd-panel-downloads" aria-labelledby="pd-tab-downloads" tabIndex={0} className="pdTabContent"><div className="pdDownloads">{p.datasheet_url?<Link href={p.datasheet_url} target="_blank"><FileText/><div><b>Product Datasheet</b><span>Open technical datasheet</span></div><Download/></Link>:null}{p.installation_guide_url?<Link href={p.installation_guide_url} target="_blank"><FileText/><div><b>Installation Guide</b><span>Open installation instructions</span></div><Download/></Link>:null}{!p.datasheet_url&&!p.installation_guide_url&&<p>No downloads have been uploaded yet.</p>}</div></div>}
    </section>
