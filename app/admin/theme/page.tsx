...
export default function ThemeAdmin(){
  ...
  async function uploadLogo(key:BrandKey,file?:File|null){if(!file)return;if(!file.type.startsWith('image/')){setMessage('Only image files are allowed for logo upload.');return}setUploading(key);setMessage('');const path=`logos/${key}-${Date.now()}-${safeName(file)}`;const {error}=await supabase.storage.from('theme-assets').upload(path,file,{upsert:true,cacheControl:'3600'});if(error){setUploading('');setMessage(error.message);return}const {data}=supabase.storage.from('theme-assets').getPublicUrl(path);setBrand(key,data.publicUrl);setUploading('');setMessage('Logo uploaded. Check the preview, then click Save & Publish.')}
