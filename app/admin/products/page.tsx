...
const incl = (n: any, g: any = 18) =>
  Number(n || 0) * (1 + Number(g || 0) / 100);
const margin = (sell: any, cost: any) =>
  Number(sell) > 0
    ? ((Number(sell) - Number(cost || 0)) / Number(sell)) * 100
    : 0;
...
export default function Products() {
    ...
    const [p, c, b] = await Promise.all([
      loadAdminCosts(
        ...
        supabase
          .from("products")
          .select(
            `*,categories(name,slug),product_variants(${VARIANT_FIELDS}),product_images(*)`,
          )
          .order("sort_order")
          .limit(1000),
        "variant",
  ...
  const brandName = (id: string) =>
    brands.find((b) => b.id === id)?.name || "New India Solar";
  ...
  const isLow = (p: any) =>
    (p.product_variants || []).some(
      (v: any) =>
        v.is_active &&
        Number(v.stock_qty || 0) <= Number(v.low_stock_threshold || 0),
    );
  const filtered = useMemo(
    () =>
      rows.filter((r) => {
        ...
        if (stock === "low" && !isLow(r)) return false;
        if (
          stock === "out" &&
          !(r.product_variants || []).some(
            (v: any) => v.is_active && Number(v.stock_qty || 0) <= 0,
          )
        )
          return false;
        const text =
          `${r.name} ${r.slug} ${r.short_description || ""} ${r.categories?.name || ""} ${(r.product_variants || []).map((v: any) => v.sku).join(" ")}`.toLowerCase();
    ...
    inventory: rows.reduce(
      (a, p) =>
        ...
        (p.product_variants || []).reduce(
          (x: number, v: any) => x + Number(v.stock_qty || 0),
          0,
        ),
      0,
    ),
  ...
  function setV(i: number, k: string, v: any) {
    setForm((f: any) => ({
      ...f,
      variants: f.variants.map((x: any, n: number) =>
        n === i ? { ...x, [k]: v } : x,
      ),
    }));
  }
  function setI(i: number, k: string, v: any) {
    setForm((f: any) => ({
      ...f,
      images: f.images.map((x: any, n: number) =>
        n === i ? { ...x, [k]: v } : x,
      ),
    }));
  }
  ...
  function edit(p: any, duplicate = false) {
    setForm({
      ...
      status: duplicate ? "draft" : p.status || "draft",
      hsn_code: p.hsn_code || "",
      gst_rate: Number(p.gst_rate || 18),
      warranty_months: p.warranty_months || 12,
      featured: duplicate ? false : !!p.featured,
      ...
      variants: (p.product_variants?.length
        ? p.product_variants
        : [{ ...blankVariant, sku: `NIS-${(p.slug || "SKU").toUpperCase()}` }]
      ).map((v: any) => ({
        ...v,
        id: duplicate ? undefined : v.id,
        sku: duplicate ? `${v.sku}-COPY` : v.sku,
      })),
      images: [...(p.product_images || [])]
        .sort((a: any, b: any) => a.sort_order - b.sort_order)
        .map((x: any) => ({ ...x, id: duplicate ? undefined : x.id })),
    });
    setTab("overview");
    setShow(true);
    setMsg("");
  ...
  async function upload(e: ChangeEvent<HTMLInputElement>) {
    ...
    const path = `products/${slugify(form.slug || form.name || "product")}/${Date.now()}-${safe(file)}`;
    ...
    setUploading(false);
    setMsg("Image uploaded. Save the product to publish it.");
  }
  function removeImage(i: number) {
    setForm((f: any) => ({
      ...f,
      images: f.images.filter((_: any, n: number) => n !== i),
    }));
  }
  async function save(e: FormEvent) {
    ...
    const payload = {
      ...
      product_badges: lines(form.product_badges),
      ...
      installation_guide_url: form.installation_guide_url || null,
    ...
    const keepIds = form.variants
      .filter((v: Variant) => v.id)
      .map((v: Variant) => v.id);
    for (const v of form.variants.filter((v: Variant) => v.sku && v.title)) {
      const vp = {
    ...
    if (!err) {
      for (const old of current.filter((x: any) => !keepIds.includes(x.id))) {
        await supabase
          .from("product_variants")
  ...
  async function archive(p: any) {
    if (
      !confirm(
        `Set ${p.name} to inactive? It will stop appearing as an active catalogue item.`,
      )
    )
      return;
  ...
  async function applyBulk() {
    ...
    if (
      !confirm(
        `Change ${selected.length} selected product(s) to ${bulkStatus}?`,
      )
    )
      return;
    const { error } = await supabase
      ...
      .in("id", selected);
    setMsg(error ? error.message : `${selected.length} products updated.`);
    load();
  }
  function exportCsv() {
    const data = filtered.flatMap((p) =>
      (p.product_variants?.length ? p.product_variants : [{}]).map((v: any) => [
        ...
        margin(v.selling_price, v.cost_price).toFixed(2),
      ]),
    );
    const header = [
      "Product",
    ...
    a.download = `new-india-solar-products-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  return (
    <div className="catalogueV2Page">
      <div className="catalogueV2Hero">
        <div>
          <span className="adminEyebrow">CATALOGUE MASTER</span>
          <h1>Products</h1>
          <p>
            Sellable catalogue, variants, pricing, GST, stock, technical content
            and publishing control.
          </p>
        </div>
        <div className="catalogueHeroActions">
          <button onClick={load} className="catalogueBtn ghost">
            <RefreshCw size={16} />
            Refresh
          </button>
          <button onClick={exportCsv} className="catalogueBtn ghost">
            <Download size={16} />
            Export CSV
          </button>
          <Link href="/admin/products/bulk" className="catalogueBtn ghost">
            <Edit3 size={16} />
            Bulk Edit
          </Link>
          <button onClick={addNew} className="catalogueBtn">
            <PackagePlus size={16} />
            Add Product
          </button>
        </div>
      ...
      <div className="catalogueStats">
        ...
        <div>
          ...
          <b>{stats.inventory.toLocaleString("en-IN")}</b>
        </div>
      </div>
      ...
      <section className="cataloguePanel">
        <div className="catalogueTableWrap">
          <table className="catalogueTable">
            <thead>
              <tr>
                <th className="check">
                  <input
                    ...
                    checked={
                      filtered.length > 0 &&
                      filtered.every((x) => selected.includes(x.id))
                    }
                    onChange={toggleAll}
                  />
                </th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={8} className="catalogueEmpty">
                    Loading catalogue…
                  </td>
                </tr>
              ) : filtered.length ? (
                filtered.map((p) => {
                  ...
                  const low = isLow(p);
                  return (
                    <tr key={p.id} className={low ? "lowRow" : ""}>
                      <td className="check">
                        <input
                          type="checkbox"
                      ...
                      <td>
                        <div className="catalogueProductCell">
                          {firstImg(p) ? (
                            <img src={firstImg(p)} alt="" />
                          ) : (
                            <div className="catalogueNoImage">
                              <ImagePlus size={17} />
                            </div>
                          )}
                          <div>
                            <b>{p.name}</b>
                            <span>{v.sku || p.slug}</span>
                            {p.featured && (
                              <em>
                      ...
                      <td>
                        ...
                        <span className="cellSub">
                          Cost {money(v.cost_price)}
                        </span>
                      </td>
                      <td>
                        <span
                          className={`marginChip ${m < 15 ? "danger" : m < 25 ? "warn" : "good"}`}
                        >
                          {m.toFixed(1)}%
                        </span>
                      </td>
                      <td>
                        <b className={low ? "stockLow" : "cellMain"}>
                          {Number(v.stock_qty || 0)} {v.unit || "pcs"}
                        </b>
                        <span className="cellSub">
                          Alert ≤ {Number(v.low_stock_threshold || 0)}
                        </span>
                      </td>
                      <td>
                        <span className={`catalogueStatus ${p.status}`}>
                          {p.status}
                        </span>
                      </td>
                      <td>
        ...
        >
          <form className="catalogueDrawer" onSubmit={save}>
            ...
            <div className="catalogueDrawerBody">
              {tab === "overview" && (
                <div className="catalogueFormGrid">
                  ...
                  <label>
                    ...
                    >
                      <option value="">New India Solar</option>
                      {brands.map((b) => (
              ...
              {tab === "pricing" && (
                <div>
                  <div className="variantHeader">
                    <div>
                      <h3>Variants, Commercials & Inventory</h3>
                      <p>
                        Purchase cost is admin-only. Storefront uses selling
                        price and GST.
                      </p>
                    </div>
                    <button
                      ...
                      onClick={() =>
                        setForm((f: any) => ({
                          ...
                          variants: [
                            ...
                            {
                              ...blankVariant,
                              sku: `NIS-${(f.slug || "SKU").toUpperCase()}-${f.variants.length + 1}`,
                            },
                          ],
                        }))
                      }
                  ...
                  <div className="variantListV2">
                    {form.variants.map((v: Variant, i: number) => {
                      const m = margin(v.selling_price, v.cost_price);
                      return (
                        <div className="variantV2" key={v.id || i}>
                          <div className="variantTitle">
                            ...
                            <label>
                              <input
                                ...
                                checked={v.is_active}
                                onChange={(e) =>
                                  setV(i, "is_active", e.target.checked)
                                }
                              />{" "}
                              Active
                            </label>
                          ...
                          <div className="variantFields">
                            ...
                            <label>
                              ...
                              <input
                                ...
                                value={v.cost_price}
                                onChange={(e) =>
                                  setV(i, "cost_price", e.target.value)
                                }
                              />
                            </label>
                            <label>
                              <span>Selling Ex GST</span>
                              <input
                                required
                                type="number"
                                step="0.01"
                                ...
                                onChange={(e) =>
                                  setV(i, "selling_price", e.target.value)
                                }
                              />
                              <small>
                                {money(incl(v.selling_price, form.gst_rate))}{" "}
                                incl. GST
                              </small>
                            </label>
                            <label>
                            ...
                            <label>
                              ...
                              <div
                                className={`metricField ${m < 15 ? "danger" : m < 25 ? "warn" : "good"}`}
                              >
                                {m.toFixed(1)}%
                              </div>
                            </label>
                            <label>
                              ...
                              <input
                                ...
                                value={v.stock_qty}
                                onChange={(e) =>
                                  setV(i, "stock_qty", e.target.value)
                                }
                              />
                            </label>
                            <label>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setV(i, "low_stock_threshold", e.target.value)
                                }
                              />
                            </label>
                            <label>
                            ...
                            <label>
                              ...
                              <input
                                ...
                                value={v.weight_kg}
                                onChange={(e) =>
                                  setV(i, "weight_kg", e.target.value)
                                }
                              />
                            </label>
                          </div>
              ...
              {tab === "media" && (
                <div className="mediaStudioV2">
                  <div className="uploadCard">
                    <ImagePlus size={30} />
                    <h3>Product gallery</h3>
                    <p>
                      Upload clear product photographs. First image becomes the
                      primary image.
                    </p>
                    <label className="catalogueBtn">
                      <Upload size={16} />
                  ...
                  <div className="mediaGridV2">
                    {form.images.map((img: Img, i: number) => (
                      <div className="mediaCardV2" key={img.id || i}>
                        <div>
                          {img.image_url ? (
                        ...
                        <input
                          value={img.image_url}
                          onChange={(e) => setI(i, "image_url", e.target.value)}
                          placeholder="Image URL"
                        />
                        <input
                          value={img.alt_text}
                          onChange={(e) => setI(i, "alt_text", e.target.value)}
              ...
              {tab === "technical" && (
                <div className="catalogueFormGrid">
                  <label className="full">
                    <span>Technical Specifications (JSON)</span>
                    <textarea
                      rows={12}
                      value={form.specifications}
                      onChange={(e) => set("specifications", e.target.value)}
                      placeholder='{"Rated Voltage":"600V","Current":"32A"}'
                    />
                  </label>
                  <label className="full">
                  ...
                  <label className="full">
                    <span>Product Badges — one per line</span>
                    <textarea
                      rows={4}
                      value={form.product_badges}
                      onChange={(e) => set("product_badges", e.target.value)}
                    />
                  </label>
                  <label className="wide">
                  ...
                  <label className="wide">
                    <span>Installation Guide URL</span>
                    <input
                      value={form.installation_guide_url}
                      onChange={(e) =>
                        set("installation_guide_url", e.target.value)
                      }
                    />
                  </label>
                </div>
                ...
                <div className="catalogueFormGrid">
                  ...
                  <label className="full">
                    ...
                    <input
                      ...
                      maxLength={70}
                    />
                    <small>{form.seo_title.length}/70</small>
                  </label>
                  <label className="full">
                    ...
                    <textarea
                      ...
                      maxLength={180}
                    />
                    <small>{form.seo_description.length}/180</small>
                  </label>
                  <div className="seoPreview full">
                    <small>SEARCH PREVIEW</small>
                    <b>{form.seo_title || form.name || "Product title"}</b>
                    <span>
                      newindiasolar.com/product/{form.slug || "product-slug"}
                    </span>
                    <p>
