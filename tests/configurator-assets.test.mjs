...
test('all 34 source PNGs match the recorded asset hashes',()=>{
  assert.equal(pack.assets.length,34)
...
for(const code of ['ACDB','DCDB']){
 ...
 test(`${code}: every SPD/breaker combination occupies all 11 distinct slots`,()=>{
   assert.equal(spd.configurator_option_values.length,code==='ACDB'?9:8);assert.equal(breaker.configurator_option_values.length,code==='ACDB'?4:3)
