...
async function isolated(page){await page.route('**/*.supabase.co/**',async route=>{const r=route.request();if(r.method()!=='GET'&&r.method()!=='OPTIONS'){await route.abort();throw new Error('Unexpected production write: '+r.url())}await route.fulfill({contentType:'application/json',headers:{'access-control-allow-origin':'*'},body:r.headers().accept?.includes('vnd.pgrst.object')?'null':'[]'})})}
const stage=page=>page.getByTestId('builder-stage')
async function loaded(page,code){await page.goto('/customize/'+code.toLowerCase());await expect(stage(page)).toBeVisible();await expect(page.locator('.bbLayer').filter({has:page.locator('svg')})).toHaveCount(22)}
...
for(const code of ['ACDB','DCDB'])for(const width of [390,1440])test(`${code} at ${width}: all 24 SPD and breaker previews`,async({page})=>{
 ...
 expect(n).toBe(24);await page.screenshot({path:`test-results/${code}-${width}-buyer-page.png`,fullPage:true})
})
...
test('optional layers, gland quantities, save, reload, restore and reset',async({page})=>{
 ...
 await page.reload();await expect(page.getByRole('button',{name:'Restore draft',exact:true})).toBeVisible();await page.getByRole('button',{name:'Restore draft',exact:true}).click();await expect(stage(page).locator('[data-asset="wire-red-left"]')).toHaveCount(0);await expect(stage(page).locator('[data-asset="cable-gland"]')).toHaveCount(1)
...
test('RFQ carries the exact draft, and a synthetic submission is intercepted',async({page})=>{
 ...
 await page.locator('input[name=name]').fill('ISOLATED QA — never sent');await page.locator('input[name=mobile]').fill('9000000000');await page.getByRole('button',{name:'Submit Bulk Requirement'}).click();await expect(page.getByRole('heading',{name:'Requirement received.'})).toBeVisible();expect(saved.additional_requirement).toContain('ACDB-04');expect(saved.product_interest).toBe('ACDB')
})
...
test('missing image shows failure and refresh restores it; PNG failure is visible',async({page})=>{
 await isolated(page);let fail=true;await page.route('**/ac-spd-oreit.png',route=>fail?route.fulfill({status:404,body:'Unavailable'}):route.continue());await loaded(page,'ACDB');await expect(stage(page).locator('.cvAssetMissing')).toBeVisible();await page.locator('.bbPreviewFoot').getByRole('button',{name:'Download PNG'}).click();await expect(page.locator('.bbNotice')).toContainText('could not be loaded');fail=false;await page.locator('.bbStatus').getByRole('button').click();await expect(stage(page).locator('.cvAssetMissing')).toHaveCount(0)
})
test('normal landing links open both quotation builders',async({page})=>{
 await isolated(page);await page.goto('/customize');await page.getByRole('link',{name:'Open ACDB Builder'}).click();await expect(stage(page)).toBeVisible();await page.goto('/customize');await page.getByRole('link',{name:'Open DCDB Builder'}).click();await expect(stage(page)).toBeVisible()
})
