...
async function install(page,options={}){
 ...
 await page.route('**/*.supabase.co/**',async route=>{
  ...
  if(state.realPhoto&&request.url()===state.realPhoto){reads.push(request.url());return route.continue()}
  let rows=[]
...
test('gallery sorts images and supports buttons, thumbnails, Home/End and wraparound',async({page})=>{
 ...
 await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-a.svg')
 await page.getByRole('button',{name:'Next image',exact:true}).click();await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-b.svg')
 await page.getByRole('button',{name:'Show product image 1',exact:true}).click();await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-a.svg')
 await page.locator('.p28Photo').focus();await page.keyboard.press('End');await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-b.svg')
 await page.keyboard.press('Home');await page.keyboard.press('ArrowLeft');await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-b.svg')
 await page.keyboard.press('ArrowRight');await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-a.svg')
})
test('enlarged gallery keyboard navigation, focus containment, Escape and scroll restoration',async({page})=>{
 ...
 await page.keyboard.press('End');await expect(dialog.locator('img')).toHaveAttribute('src','/gallery-b.svg')
 await page.keyboard.press('Home');await expect(dialog.locator('img')).toHaveAttribute('src','/gallery-a.svg')
...
test('vertical, cancelled and multi-touch gestures do not change images; sideways swipes do',async({page})=>{
 await install(page);await open(page)
 for(const kind of ['vertical','cancel','multi']){await gesture(page,'.p28Photo',kind);await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-a.svg')}
 await gesture(page,'.p28Photo','horizontal');await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-b.svg')
 await page.getByRole('button',{name:'Enlarge product image',exact:true}).click();await gesture(page,'.p28ZoomImage','horizontal');await expect(page.locator('.p28ZoomImage img')).toHaveAttribute('src','/gallery-a.svg')
})
test('missing image shows a retry and does not poison the next photo',async({page})=>{
 ...
 await page.getByRole('button',{name:'Next image',exact:true}).click();await readyPhoto(page);await expect(page.locator('.p28Photo img')).toHaveAttribute('src','/gallery-b.svg')
...
test('mobile purchase bar reserves footer space and cleans up on resize and route change',async({page})=>{
 ...
 expect(await page.evaluate(()=>document.querySelector('.nisFooterBottom').getBoundingClientRect().bottom<=document.querySelector('.p28MobileBar').getBoundingClientRect().top+2)).toBe(true)
...
// These two URLs were read from active product-image records. No catalogue records are edited.
const realPhotos=[
...
for(const [i,sample] of realPhotos.entries())test(`read-only existing catalogue photograph ${i+1}`,async({page})=>{
 ...
 // Emit a compact review image through the CI log; the full screenshot stays in the artifact.
 const buffer=await page.screenshot({type:'png'})
 const preview=await require('sharp')(buffer).resize({width:width===1440?720:330}).jpeg({quality:42}).toBuffer()
 console.log(`GALLERY_VISUAL_REVIEW_${width}=${preview.toString('base64')}`)
})
