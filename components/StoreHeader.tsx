  ...
  <header className="nhHeader" ref={headerRef}>
   <div className="container nhNav">
    ...
    <nav className="nhDesktopNav" aria-label="Primary navigation">
     ...
     <Link href="/shop?q=ACDB">ACDB</Link><Link href="/shop?q=DCDB">DCDB</Link>
   ...
   {mega==='customize'&&<div className="nhMega" id="customize-mega-menu"><div className="container nhMegaCustom"><div className="nhCustomIntro"><span>CUSTOM CONFIGURATION</span><h2>Build the distribution box your site needs.</h2><p>Select enclosure, protection devices, wiring and accessories. Review the configuration and BOM before ordering.</p><Link href="/shop" onClick={closeAll}>Or shop ready products <ArrowRight size={15}/></Link></div><Link className="nhBuildCard dark" href="/customize/acdb" onClick={closeAll}><div><span>ACDB BUILDER</span><i>AC</i></div><h3>Build Your ACDB</h3><p>System size • Phase • MCB/MCCB • SPD • Busbar • Wiring</p><strong>Start builder <ArrowRight size={16}/></strong></Link><Link className="nhBuildCard green" href="/customize/dcdb" onClick={closeAll}><div><span>DCDB BUILDER</span><i>DC</i></div><h3>Build Your DCDB</h3><p>Strings • Voltage • DC MCB • SPD • Fuse • Cable • Glands</p><strong>Start builder <ArrowRight size={16}/></strong></Link></div></div>}
  </header>
  {mobileOpen&&<div className="nhMobileOverlay" role="presentation" onClick={()=>setMobileOpen(false)}><aside className="nhMobileDrawer" id="buyer-mobile-navigation" role="dialog" aria-modal="true" aria-label="Buyer navigation" onClick={event=>event.stopPropagation()}><div className="nhMobileHead"><img src={mobileLogo} alt={branding.logoAlt}/><button aria-label="Close navigation" onClick={()=>setMobileOpen(false)}><X size={21}/></button></div><button className="nhMobileSearch" onClick={()=>{setMobileOpen(false);setSearchOpen(true)}}><Search size={18}/> Search products <span>⌘K</span></button><div className="nhMobileJourneys" aria-label="Quick buying actions"><Link href="/shop" onClick={closeAll}><ShoppingCart size={17}/><span><b>Buy Products</b><small>Browse catalogue</small></span></Link><Link href="/customize" onClick={closeAll}><Settings2 size={17}/><span><b>Build a Box</b><small>ACDB / DCDB</small></span></Link><Link href="/bulk-order" onClick={closeAll}><FileText size={17}/><span><b>Project RFQ</b><small>Bulk pricing</small></span></Link></div><nav className="nhMobileNav" aria-label="Mobile navigation"><button aria-expanded={mobileSection==='products'} onClick={()=>setMobileSection(mobileSection==='products'?null:'products')}>Products <ChevronDown size={16}/></button>{mobileSection==='products'&&<div className="nhMobileSub">{productGroups.map(({name,href})=><Link key={name} href={href} onClick={closeAll}>{name}<ArrowRight size={13}/></Link>)}<Link className="all" href="/shop" onClick={closeAll}>View all products <ArrowRight size={13}/></Link></div>}<button aria-expanded={mobileSection==='customize'} onClick={()=>setMobileSection(mobileSection==='customize'?null:'customize')}>ACDB / DCDB Builder <ChevronDown size={16}/></button>{mobileSection==='customize'&&<div className="nhMobileSub nhMobileBuilders"><Link href="/customize/acdb" onClick={closeAll}>Build ACDB <ArrowRight size={13}/></Link><Link href="/customize/dcdb" onClick={closeAll}>Build DCDB <ArrowRight size={13}/></Link></div>}{headerLinks.map(item=><span className="nhMobileCmsItem" key={item.id}>{navigationLink(item)}</span>)}<Link href="/account" onClick={closeAll}>My Account <User size={16}/></Link><Link href="/cart" onClick={closeAll}>Cart <span>{count}</span></Link></nav><div className="nhMobileSupport"><Headphones size={18}/><span><b>Need help selecting components?</b><small>Send your requirement through Project RFQ.</small></span><Link href="/bulk-order" onClick={closeAll}>Ask</Link></div><div className="nhMobileFoot"><Building2 size={18}/><div><b>New India Solar Components Pvt Ltd</b><span>{branding.tagline}</span></div></div></aside></div>}
