...
export default function StoreHeader(){
  ...
  return <>
    ...
    {open&&<div className="nisDrawerWrap" onClick={()=>setOpen(false)}><aside className="nisDrawer" onClick={e=>e.stopPropagation()}><div className="nisDrawerHead"><Image src="/new-india-solar-logo.webp" alt="New India Solar" width={165} height={90}/><button onClick={()=>setOpen(false)} aria-label="Close menu"><X/></button></div><nav><Link href="/shop">Shop All Products</Link><Link href="/customize/acdb">Build ACDB</Link><Link href="/customize/dcdb">Build DCDB</Link><Link href="/bulk-order">Bulk / Project Requirement</Link><Link href="/account">My Account</Link><Link href="/cart">Cart ({count})</Link></nav><div className="nisDrawerFoot">ACDB • DCDB • BOS Components<br/><small>New India Solar Components Pvt Ltd</small></div></aside></div>}
