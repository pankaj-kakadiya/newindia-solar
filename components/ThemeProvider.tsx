...
export type StoreTheme={
  branding:{companyName:string;tagline:string;promise:string;logoUrl:string;headerLogoUrl:string;footerLogoUrl:string;mobileLogoUrl:string;faviconUrl:string;emailLogoUrl:string;invoiceLogoUrl:string;logoAlt:string}
  ...
  layout:{containerWidth:number;sectionSpacing:number;cardRadius:number;buttonRadius:number;inputRadius:number;headerHeight:number;logoWidth:number;footerLogoWidth:number;mobileLogoWidth:number;shadowOpacity:number}
  header:{showTopbar:boolean;sticky:boolean;topbarItems:string[]}
  customCss:string
}

export const defaultTheme:StoreTheme={
  branding:{companyName:'New India Solar Components Pvt Ltd',tagline:'Powering India’s Solar Installations.',promise:'Tested. Packed. Guaranteed.',logoUrl:'/new-india-solar-logo.webp',headerLogoUrl:'/new-india-solar-logo.webp',footerLogoUrl:'/new-india-solar-logo.webp',mobileLogoUrl:'/new-india-solar-logo.webp',faviconUrl:'/new-india-solar-logo.webp',emailLogoUrl:'/new-india-solar-logo.webp',invoiceLogoUrl:'/new-india-solar-logo.webp',logoAlt:'New India Solar Components Pvt Ltd'},
...
export function mergeTheme(raw:any):StoreTheme{
  ...
  if(!branding.faviconUrl)branding.faviconUrl=branding.logoUrl
  if(!branding.emailLogoUrl)branding.emailLogoUrl=branding.logoUrl
  if(!branding.invoiceLogoUrl)branding.invoiceLogoUrl=branding.logoUrl
  return {
    ...defaultTheme,
    ...raw,
    branding,
...
.nisNav{height:${l.headerHeight}px!important}.nisBrand img{width:${l.logoWidth}px!important;max-height:${Math.max(44,l.headerHeight-18)}px!important;object-fit:contain!important}.nisDrawerHead img{width:${l.mobileLogoWidth}px!important;height:auto!important}.nisFooterLogo img{width:${l.footerLogoWidth}px!important;max-width:100%!important;height:auto!important;object-fit:contain!important}
...
@media(max-width:760px){.nisBrand img{width:${l.mobileLogoWidth}px!important;max-height:${Math.max(40,l.headerHeight-20)}px!important}.nisBrand{min-width:${Math.min(l.mobileLogoWidth+20,220)}px!important}.nisFooterLogo img{width:${Math.min(l.footerLogoWidth,230)}px!important}}
${theme.customCss||''}
  `
}
...
export default function ThemeProvider({children}:{children:React.ReactNode}){
  ...
  useEffect(()=>{if(path.startsWith('/admin')){document.getElementById('nis-dynamic-theme')?.remove();return}let tag=document.getElementById('nis-dynamic-theme') as HTMLStyleElement|null;if(!tag){tag=document.createElement('style');tag.id='nis-dynamic-theme';document.head.appendChild(tag)}tag.textContent=css(theme);let icon=document.querySelector("link[rel='icon']") as HTMLLinkElement|null;if(theme.branding.faviconUrl){if(!icon){icon=document.createElement('link');icon.rel='icon';document.head.appendChild(icon)}icon.href=theme.branding.faviconUrl}return()=>{}},[theme,path])
