...
import QuantityControl from './buyer/QuantityControl'
...
export default function ConfiguratorBuilder({slug,title,code,assetPreview=false}:{slug:'custom-acdb'|'custom-dcdb';title:string;code:BuilderCode;assetPreview?:boolean}){
 ...
 function quantity(g:any,id:string,next:number){dirty.current=true;setReference('');setSelection(s=>({...s,[g.option_key]:(s[g.option_key]||[]).map(p=>p.id===id?{...p,qty:Math.max(Number(g.min_quantity)||1,Math.min(Number(g.max_quantity)||1,next))}:p)}))}
