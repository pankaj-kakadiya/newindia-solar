...
export default function AccountSecurityPanel({setupOnly=false,onComplete}:{setupOnly?:boolean;onComplete?:()=>void|Promise<void>}){
  ...
  async function load(){
    ...
    if(!setupOnly&&needsMfaChallenge(mfa))router.replace('/admin/login?next=%2Fadmin%2Faccount-security')
  ...
  return <section>{!setupOnly&&<div className="adminPageHead"><div><span className="adminEyebrow">ACCOUNT SECURITY</span><h1>Login & Device Security</h1><p>Protect your account with an authenticator and manage device sessions.</p></div><button className="btn" disabled={busy} onClick={refresh}>Refresh</button></div>}
    ...
    {failed?<button className="btn" onClick={refresh}>Retry security check</button>:!state?<p>Loading account security…</p>:<>
      {!setupOnly&&<div className="adminSecurityCard"><h2>{state.currentLevel==='aal2'?'Session protected by 2FA':'Password-only session'}</h2><p>{verifiedCount} verified authenticator{verifiedCount===1?'':'s'}{policy?' · 2FA required by your administrator':''}</p></div>}
      {challenge?<p>Continue verification on the login screen.</p>:<>
