...
import {BarChart3,Bell,Boxes,ChevronDown,ChevronLeft,ChevronRight,Command,ExternalLink,Factory,FileText,Globe2,IndianRupee,KeyRound,LayoutDashboard,LogOut,Menu,Package,Palette,Search,Settings,ShieldCheck,ShoppingCart,SlidersHorizontal,Users,Warehouse,X} from 'lucide-react'
...
const navGroups:NavGroup[]=[
 {label:'Overview',items:[{href:'/admin',label:'Dashboard',icon:LayoutDashboard,module:'dashboard'},{href:'/admin/reports',label:'Reports & Analytics',icon:BarChart3,module:'reports'}]},
...
export default function AdminShell({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{if(isLogin)return;let alive=true;(async()=>{
   ...
   const next:Record<BadgeKey,number>={orders:0,production:0,rfqs:0}
 ...
 useEffect(()=>{setMobileOpen(false);setNotificationsOpen(false);setActionsOpen(false);setProfileOpen(false);setSearchOpen(false);setSearch('')},[pathname])
 ...
 const totalAlerts=badges.orders+badges.production+badges.rfqs,displayName=email==='Admin'?'Admin':email.split('@')[0].replace(/[._-]+/g,' '),roleLabel=adminRoleLabels[access?.admin_role||access?.role||'admin']||access?.admin_role||access?.role||'Staff'
 ...
 return <div className={`adminV2 ${collapsed?'isCollapsed':''}`}>
  ...
  <div className="adminV2Main"><header className="adminV2Topbar"><div className="adminV2TopbarLeft"><button className="adminV2MenuButton" onClick={()=>setMobileOpen(true)} aria-label="Open navigation"><Menu size={20}/></button><div className="adminV2PageIdentity"><small>New India Solar</small><b>{routeTitle(pathname)}</b></div></div><div className="adminV2TopbarRight">
   ...
   <div className="adminV2DropWrap"><button className="adminV2QuickButton" onClick={()=>setActionsOpen(v=>!v)}><span>Quick Actions</span><ChevronDown size={15}/></button>{actionsOpen&&<div className="adminV2Dropdown adminV2QuickMenu">
    {canAdmin(access,'content')&&<Link href="/admin/content">Website content <span>→</span></Link>}{canAdmin(access,'reports')&&<Link href="/admin/reports">Reports & analytics <span>→</span></Link>}{canAdmin(access,'products')&&<Link href="/admin/products">Manage products <span>→</span></Link>}{canAdmin(access,'purchasing')&&<Link href="/admin/purchasing">Open purchasing <span>→</span></Link>}{canAdmin(access,'pricing')&&<Link href="/admin/pricing">Pricing & margins <span>→</span></Link>}{canAdmin(access,'finance')&&<Link href="/admin/finance">Finance & GST <span>→</span></Link>}{canAdmin(access,'configurator')&&<Link href="/admin/configurator">Open configurator <span>→</span></Link>}{canAdmin(access,'orders')&&<Link href="/admin/orders">Review orders <span>→</span></Link>}{canAdmin(access,'security')&&<Link href="/admin/access">Roles & audit <span>→</span></Link>}
   </div>}</div>
   <div className="adminV2DropWrap"><button className="adminV2IconButton" onClick={()=>setNotificationsOpen(v=>!v)} aria-label="Notifications"><Bell size={18}/>{totalAlerts>0&&<span className="adminV2AlertDot">{totalAlerts>9?'9+':totalAlerts}</span>}</button>{notificationsOpen&&<div className="adminV2Dropdown adminV2Notifications"><div className="adminV2DropdownHead"><b>Needs attention</b><small>{totalAlerts} open</small></div>{canAdmin(access,'orders')&&<Link href="/admin/orders"><span><ShoppingCart size={16}/> Open orders</span><b>{badges.orders}</b></Link>}{canAdmin(access,'production')&&<Link href="/admin/production"><span><Factory size={16}/> Production queue</span><b>{badges.production}</b></Link>}{canAdmin(access,'rfqs')&&<Link href="/admin/rfqs"><span><FileText size={16}/> Active RFQs</span><b>{badges.rfqs}</b></Link>}</div>}</div>
  ...
  {searchOpen&&<div className="adminV2CommandBackdrop" onMouseDown={()=>setSearchOpen(false)}><div className="adminV2Command" onMouseDown={e=>e.stopPropagation()} role="dialog" aria-modal="true"><div className="adminV2CommandInput"><Search size={20}/><input ref={searchRef} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search permitted pages and tools…"/><button onClick={()=>setSearchOpen(false)}>ESC</button></div><div className="adminV2CommandResults">{filteredNav.length?filteredNav.map(item=>{const Icon=item.icon;return <Link href={item.href} key={item.href}><span className="adminV2CommandIcon"><Icon size={17}/></span><span><b>{item.label}</b><small>{item.group}</small></span><ChevronRight size={16}/></Link>}):<div className="adminV2CommandEmpty">No permitted admin page matches “{search}”.</div>}</div><div className="adminV2CommandFooter"><span>Role: {roleLabel}</span><span>⌘K anytime · Esc to close</span></div></div></div>}
 </div>
}
