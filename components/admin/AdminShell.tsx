...
const navGroups:NavGroup[]=[
 {label:'Overview',items:[{href:'/admin',label:'Dashboard',icon:LayoutDashboard,module:'dashboard'},{href:'/admin/notifications',label:'Notifications & Tasks',icon:Bell,badge:'alerts',module:'notifications'},{href:'/admin/reports',label:'Reports & Analytics',icon:BarChart3,module:'reports'}]},
...
export default function AdminShell({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{if(isLogin)return;let alive=true;(async()=>{
   ...
   if(canAdmin(a,'notifications'))tasks.push((async()=>{await supabase.rpc('refresh_workflow_notifications');const {data}=await supabase.rpc('workflow_notification_feed',{p_limit:6});const rows=(data||[]) as WorkflowAlert[];if(alive){setWorkflowAlerts(rows);next.alerts=rows.filter(x=>!x.is_read).length}})())
