...
const navGroups: NavGroup[] = [
  ...
  {
    label: 'Operations',
    items: [{ href: '/admin/production', label: 'Production', icon: Factory, badge: 'production' }],
  },
  {
    label: 'Settings',
    items: [
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  function toggleCollapsed() {
    setCollapsed((value) => {
      const next = !value
      window.localStorage.setItem('nis-admin-sidebar-collapsed', next ? '1' : '0')
      return next
    })
  }
    ...
    <div className={`adminV2 ${collapsed ? 'isCollapsed' : ''}`}>
      ...
      <aside className={`adminV2Sidebar ${mobileOpen ? 'mobileOpen' : ''}`}>
        <div className="adminV2BrandRow">
          <Link href="/admin" className="adminV2Brand" aria-label="New India Solar admin dashboard">
            <span className="adminV2LogoBox">
              <img src="/new-india-solar-full-logo.webp" alt="New India Solar" />
            </span>
          ...
          <button className="adminV2MobileClose" onClick={() => setMobileOpen(false)} aria-label="Close navigation">
            <X size={18} />
          </button>
        </div>
            ...
            <section className="adminV2NavGroup" key={group.label}>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <span className="adminV2NavIcon"><Icon size={18} strokeWidth={1.9} /></span>
                    <span className="adminV2NavLabel">{item.label}</span>
                    {count > 0 && <span className="adminV2NavBadge">{count > 99 ? '99+' : count}</span>}
                  </Link>
                )
              })}
            </section>
        ...
        <div className="adminV2SidebarFooter">
          <a className="adminV2StoreLink" href="/" target="_blank" rel="noreferrer" title="Open storefront">
            <ExternalLink size={17} />
            <span>View Store</span>
          </a>
          <button className="adminV2Collapse" onClick={toggleCollapsed} title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
      ...
      <div className="adminV2Main">
        <header className="adminV2Topbar">
          <div className="adminV2TopbarLeft">
            <button className="adminV2MenuButton" onClick={() => setMobileOpen(true)} aria-label="Open admin navigation">
              <Menu size={20} />
            </button>
          ...
          <div className="adminV2TopbarRight">
            <button className="adminV2SearchButton" onClick={() => setSearchOpen(true)}>
              <Search size={17} />
            ...
            <div className="adminV2DropWrap">
              <button className="adminV2QuickButton" onClick={() => setActionsOpen((v) => !v)} aria-expanded={actionsOpen}>
            ...
            <div className="adminV2DropWrap">
              <button className="adminV2IconButton" onClick={() => setNotificationsOpen((v) => !v)} aria-label="Notifications" aria-expanded={notificationsOpen}>
                <Bell size={18} />
                {totalAlerts > 0 && <span className="adminV2AlertDot">{totalAlerts > 9 ? '9+' : totalAlerts}</span>}
              </button>
              {notificationsOpen && (
            ...
            <div className="adminV2DropWrap">
              <button className="adminV2ProfileButton" onClick={() => setProfileOpen((v) => !v)} aria-expanded={profileOpen}>
                <span className="adminV2Avatar">{displayName.slice(0, 1).toUpperCase()}</span>
                <span className="adminV2ProfileCopy"><b>{displayName}</b><small>{role}</small></span>
                <ChevronDown size={14} />
              </button>
              {profileOpen && (
      ...
      {searchOpen && (
        <div className="adminV2CommandBackdrop" onMouseDown={() => setSearchOpen(false)}>
              ...
              {filteredNav.length ? filteredNav.map((item) => {
                ...
                return (
                  <Link href={item.href} key={item.href}>
                    <span className="adminV2CommandIcon"><Icon size={17} /></span>
                    <span><b>{item.label}</b><small>{item.group}</small></span>
                    <ChevronRight size={16} />
                  </Link>
                )
              }) : <div className="adminV2CommandEmpty">No admin page matches “{search}”.</div>}
            </div>
