...
export default function StoreFooter(){
 ...
 const render=(n:any)=>{const label=String(n.label||'').toLowerCase().trim(),href=label==='why new india'?'/why-new-india':label==='contact'||label==='contact us'?'/contact':n.href;return n.is_external?<a key={n.id} href={href} target="_blank" rel="noreferrer">{n.label}<ExternalLink size={12}/></a>:<Link key={n.id} href={href}>{n.label}</Link>}
 return <footer className="nisFooter" id="contact"><div className="container">
  ...
  <div className="nisFooterGrid nisFooterGridV3"><div className="nisFooterBrand"><Link href="/" className="nisFooterLogo"><img width={520} height={160} loading="lazy" decoding="async" src={footerLogo} alt={b.logoAlt}/></Link><p>{footer.about}</p><div className="nisFooterTrust"><ShieldCheck size={18}/> {b.promise}</div><div className="nisCompanyCard"><b>{COMPANY.legalName}</b><span>GSTIN: {COMPANY.gstin}</span><span><MapPin size={15}/>{COMPANY.addressLine1}, {COMPANY.addressLine2}, {COMPANY.city}, {COMPANY.state} {COMPANY.postalCode}, {COMPANY.country}</span><span><Globe2 size={15}/>{COMPANY.website}</span></div></div>
   <div><h3 className="nisFooterHeading">Shop</h3><Link href="/categories">All categories</Link><Link href="/categories/acdb">ACDB</Link><Link href="/categories/dcdb">DCDB</Link><Link href="/categories/spd">Surge protection</Link><Link href="/categories/solar-cable">Solar cable</Link>{products.map(render)}</div><div><h3 className="nisFooterHeading">Build & Buy</h3><Link href="/combo">ACDB + DCDB combo</Link><Link href="/customize">Custom box builders</Link><Link href="/bulk-order">Project RFQ</Link><Link href="/account">Orders & account</Link><Link href="/downloads">Downloads</Link>{buy.map(render)}</div>
   <div><h3 className="nisFooterHeading">Company & Support</h3><Link href="/account/support">Secure chat support</Link><Link href="/why-new-india">Why New India</Link><Link href="/contact">Contact us</Link><Link href="/policies/shipping">Shipping & delivery</Link><Link href="/policies/returns">Returns & cancellation</Link><Link href="/policies/warranty">Warranty & support</Link><Link href="/policies/privacy">Privacy policy</Link><Link href="/policies/terms">Terms of use & sale</Link>{company.map(render)}{footerPages.map(p=><Link key={p.id} href={`/pages/${p.slug}`}>{p.title}</Link>)}<p className="nisFooterSmall">{footer.small_note}</p></div>
