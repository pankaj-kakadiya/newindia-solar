...
export default function SupportLauncher(){
 ...
 return <div className={`supportLauncher ${open?'open':''}`}><button type="button" className="supportLauncherButton" onClick={()=>setOpen(v=>!v)} aria-label={open?'Close support':'Open secure support'} aria-expanded={open} aria-controls="support-launcher-panel">{open?<X/>:<MessageCircle/>}<span>Support</span></button>{open&&<aside id="support-launcher-panel" role="dialog" aria-label="Secure support"><span><LockKeyhole/> End-to-end encrypted</span><h2>How can we help?</h2><p>Chat privately with New India Solar for product, order, payment, delivery or technical support.</p><Link href={signedIn?'/account/support':'/login?next=/account/support'}>{signedIn?'Open secure chat':'Sign in to chat securely'}</Link><small>Buyer access is protected automatically on this device.</small></aside>}</div>
}
