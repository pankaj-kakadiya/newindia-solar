...
export default function SupportLauncher(){
 ...
 useEffect(()=>{supabase.auth.getUser().then(({data})=>{setSignedIn(Boolean(data.user));setUserId(data.user?.id||'')});const {data}=supabase.auth.onAuthStateChange((_event,session)=>{setSignedIn(Boolean(session?.user));setUserId(session?.user?.id||'')});return()=>data.subscription.unsubscribe()},[])
 ...
 useEffect(()=>{if(!conversationId||!userId)return;const channel=supabase.channel(`support-popup-${conversationId}`).on('postgres_changes',{event:'*',schema:'public',table:'support_messages',filter:`conversation_id=eq.${conversationId}`},()=>loadMessages(conversationId)).subscribe();return()=>{supabase.removeChannel(channel)}},[conversationId,userId,loadMessages])
